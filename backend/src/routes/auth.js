...
router.post('/register', authLimiter, async (req, res) => {
  try {
    ...
    res.status(201).json({ token, user: { _id: user._id, name: user.name, email: user.email, role: user.role, specialization: user.specialization } });
  } catch (error) {
...
router.post('/login', authLimiter, async (req, res) => {
  try {
    ...
    const token = generateToken(user._id);
    res.json({ token, user: { _id: user._id, name: user.name, email: user.email, role: user.role, specialization: user.specialization } });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
});
